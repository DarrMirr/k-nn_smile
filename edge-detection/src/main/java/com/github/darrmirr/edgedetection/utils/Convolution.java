...
public class Convolution {
    ...
    public double[][] apply(Image image, double[][] filter) {
        double[][] redConv = convolutionType2(image.toRedChannel(), image.getHeight(), image.getWidth(), filter, 3, 3, 1);
        double[][] greenConv = convolutionType2(image.toGreenChannel(), image.getHeight(), image.getWidth(), filter, 3, 3, 1);
        double[][] blueConv = convolutionType2(image.toBlueChannel(), image.getHeight(), image.getWidth(), filter, 3, 3, 1);
        ...
        for (int i = 0; i < redConv.length; i++) {
            for (int j = 0; j < redConv[i].length; j++) {
                finalConv[i][j] = fixOutOfRangeRGBValues(redConv[i][j] + greenConv[i][j] + blueConv[i][j]);
            }
        }
        return finalConv;
    }
